...
from kuimaze2.map import Action, Map, Role, State
...
class SearchProblem:
    def __init__(
        ...
        """
        self.map = map
        self._start: State | None = self.map.start
...
class TkSearchView(SearchView):
    ...
    def render(
        ...
        if colors:
            ...
            self.canvas.set_square_colors_from_visited(self.env._visited)  # type: ignore
