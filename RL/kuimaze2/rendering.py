...
from kuimaze2.map import Action, Border, Cell, Map, Role, State
...
class RectCoords:
    ...
    def of_border_line(self, border: Border) -> tuple[int, int, int, int]:
        match border:
            case Border.TOP:
            ...
            case _:
                raise ValueError(f"Unexpected border value: {border}")


class MapCanvas(tk.Canvas):
    def __init__(self, parent: tk.Tk, map: Map, sq_size: int = 0, **kwargs: Any):
        if sq_size == 0:
    ...
    def draw_border(self, square: Cell) -> None:
    ...
    def draw_row_indices(self) -> None:
    ...
    def draw_col_indices(self) -> None:
    ...
    def draw_square_texts(
        self, texts: dict[State, str] | None = None, default_text: str = ""
    ) -> None:
        texts = texts or {}
        for sq in self.map:
    ...
    def set_square_text(self, position: State, text: str) -> None:
        self.itemconfig(self.texts[position], text=text)

    def update_square_texts(self, texts: dict[State, str]) -> None:
...
class SquareCanvas(MapCanvas):
    def __init__(
        self, parent: tk.Tk, map: Map, sq_size: int = 0, **kwargs: dict[str, Any]
    ):
        super().__init__(parent, map, sq_size, **kwargs)
        self.squares: dict[State, Id] = {}
        self.circles: dict[State, Id] = {}
        self.path_line: Id | None = None
        self.state_action_arrow: Id | None = None
        self.draw()
    ...
    def draw_square(self, square: Cell) -> None:
        coords = self._coords_from_position(square.position)
    ...
    def draw_circle(self, square: Cell) -> None:
        coords = self._coords_from_position(square.position)
    ...
    def draw_init_state_action_arrow(self) -> None:
        state = State(0, 0)
        action = Action.UP
    ...
    def state_action_arrow_coords(
        ...
        difvec = action.to_vec()  # this is r, c
            ...
            int(center[0] + difvec[1] * self.square_size * arrow_length),
            int(center[1] + difvec[0] * self.square_size * arrow_length),
        )  # 0-1 and 1-0 because of x + k1*c, y + k2*r
        return center + end  # tuple concatenation

    def draw_state_action_arrow(self, state: State, action: Action) -> None:
        if not self.state_action_arrow:
            raise ValueError(
                "State action arrow not initialized. Did you call 'draw_init_state_action_arrow' first?"
            )
        coords = self.state_action_arrow_coords(state, action)
        self.coords(self.state_action_arrow, coords)
        self.itemconfig(self.state_action_arrow, state=tk.NORMAL)

    def hide_state_action_arrow(self) -> None:
        if not self.state_action_arrow:
            raise ValueError(
                "State action arrow not initialized. Did you call 'draw_init_state_action_arrow' first?"
            )
    ...
    def draw_path(self, path: list[State]) -> None:
        if not self.path_line:
            raise ValueError(
                "Path line not initialized. Did you call 'draw_init_path' first?"
            )
        points: list[int] = []
        for state in path:
    ...
    def set_square_color(self, position: State, color: Color) -> None:
        self.itemconfig(self.squares[position], fill=color.to_hex())

    def set_circle_color(
        self, position: State, color: Color, visible: bool = True
    ) -> None:
        self.itemconfig(
    ...
    def set_circle_visibility(self, position: State, visible: bool = True) -> None:
        self.itemconfig(
            self.circles[position], state=tk.NORMAL if visible else tk.HIDDEN
        )

    def set_square_colors(
        self, colors: dict[State, Color], keep_role_colors: bool = False
    ) -> None:
...
class TriangleCanvas(MapCanvas):
    def __init__(self, parent: tk.Tk, map: Map, sq_size: int = 0, **kwargs: Any):
        super().__init__(parent, map, sq_size, **kwargs)
        self.triangles: dict[State, dict[Action, Id]] = {}
        self.triangle_texts: dict[tuple[State, Action], Id] = {}
        self.draw()
    ...
    def draw_triangle(self, square: Cell, action: Action) -> None:
    ...
    def _tr_coords(
        self, square: Cell, action: Action
    ...
    def set_triangle_color(self, position: State, action: Action, color: Color) -> None:
        self.itemconfig(self.triangles[position][action], fill=color.to_hex())

    def set_triangle_colors(self, colors: dict[tuple[State, Action], Color]) -> None:
        for (position, action), color in colors.items():
            self.set_triangle_color(position, action, color)

    def draw_triangle_texts(
        self, texts: dict[tuple[State, Action], str] = {}, default_text: str = ""
    ) -> None:
        texts = texts or {}
        for sq in self.map:
    ...
    def set_triangle_text(self, position: State, action: Action, text: str) -> None:
        self.itemconfig(self.triangle_texts[position, action], text=text)

    def update_triangle_texts(self, texts: dict[tuple[State, Action], str]) -> None:
...
class SearchCanvas(SquareCanvas):
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        self.current_circle: State | None = None
        self.next_circles: list[State] = []
        self.frontier_circles: list[State] = []

    def color_from_value(
        self, value: float | None = None, factor: float | None = None
    ) -> Color:
        ((min_value, color1), (max_value, color2)) = self.value_colors
        if factor is None:
            if value is None:
                raise ValueError("Either value or factor must be provided")
            if value <= min_value:
                return color1
            if value >= max_value:
                return color2
            factor = (value - min_value) / (max_value - min_value)
        color = color1.mix(color2, factor)  # type: ignore
        return color

    def set_square_colors_from_values(self, values: dict[State, float]) -> None:
        colors = {
    ...
    def set_square_colors_from_visited(self, visited: Iterable[State]) -> None:
        color = COLOR_FROM_ROLE[Role.START].mix(Color(255, 255, 255), 0.5)
        colors = {position: color for position in visited}
        self.set_square_colors(colors, keep_role_colors=True)

    def set_current_state(self, state: State | None = None) -> None:
    ...
    def set_frontier_states(self, frontier: list[State]) -> None:
        self.reset_frontier_states()
        for state in frontier:
            self.add_frontier_state(state)

    def add_frontier_state(self, state: State) -> None:
        self.set_circle_color(state, FRONTIER_CIRCLE_COLOR)
        self.set_circle_visibility(state, True)
        self.frontier_circles.append(state)

    def reset_frontier_states(self) -> None:
...
class QValueCanvas(TriangleCanvas):
    ...
    def set_triangle_colors_from_qvalues(self, qvalues: QTable):
        colors: dict[tuple[State, Action], Color] = {}
